...
export function mountSearch(query) {
	Promise.all([
		import ('client/system/crud/templates/search-template.tag.html')
	]).then(() => {
 ...
 * @param {object} params
 */
export function mountCreate(params) {
	Promise.all([
 ...
 * @param {string} msg
 */
export function mountDeadEnd(msg) {
	Promise.all([
		import ('client/system/crud/templates/dead-end-template.tag.html')
	]).then(() => {
